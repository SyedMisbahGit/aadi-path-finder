...
class RealTimeDataService {
  ...
  async getLiveCutoffs(examType: string, year: string = '2024') {
    ...
    try {
      // Try to fetch from MCC/JoSAA APIs
  ...
  async getCollegeDatabase(filters: any = {}) {
    const cacheKey = `colleges-${JSON.stringify(filters)}`;
    const cached = this.getFromCache(cacheKey);
    if (cached) return cached;

    try {
      const data = await this.fetchCollegeData(filters);
      this.setCache(cacheKey, data);
      return data;
    } catch (error) {
      console.error('College data fetch failed:', error);
  ...
  private async fetchCollegeData(filters: any) {
    const response = await fetch('/api/colleges', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(filters)
    });
    if (!response.ok) throw new Error('College API failed');
    return await response.json();
  }
